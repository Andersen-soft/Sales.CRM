...
import com.andersenlab.crm.rest.resolvers.CountryPredicateResolver;
...
public class CountryController extends BaseController {
    ...
    @ApiOperation(value = "Получение страны по id",
            notes = "id - обязательное поле(прим. /266)")
    public BaseResponse<Country> getById(@PathVariable("id") Long id) {
        return new BaseResponse<>(countryService.findById(id));
    }

    @GetMapping(produces = MediaType.APPLICATION_JSON_VALUE)
    @ApiOperation(value = "Получение списка стран с возможностью фильтрации",
            notes = "Обязательных полей нет.\n" +
                    "Возможные фильтры по полям:\n id - id страны(прим. id=4)\n name - название страны (прим. name=Афганистан)\n " +
    ...
    public BaseResponse<Page<CountryDto>> getCountries(
            @CrmPredicate(resolver = CountryPredicateResolver.class) Predicate predicate,
            Pageable pageable,
            Locale locale
    ) {
        return new BaseResponse<>(countryService.findAll(predicate, pageable, locale));
    }

    @HasRole(roles = {ROLE_SALES_HEAD, ROLE_SALES})
    @GetMapping("/get_sale_report_countries")
    @ApiPageable
    @ApiOperation(value = "Получение уникальных значений для фильтра по странам в отчёте по лидам",
            notes = "Обязательных полей нет\n" +
                    ...
                    "Задание параметров отображения результата:\n page - номер страницы результатов (прим. page=9)\n" +
                    "size - количество записей на странице (прим. size=13)\n sort - сортировка (прим. sort=id,desc)")
    public BaseResponse<Page<SaleReportCountryResponse>> getSaleReportCompanies(
            @QuerydslPredicate(root = SaleReport.class) Predicate predicate,
            Pageable pageable,
            Locale locale) {
        return new BaseResponse<>(countryService.findAllBySaleReport(predicate, pageable, locale));
    }
}
