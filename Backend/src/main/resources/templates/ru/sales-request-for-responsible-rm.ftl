...
<ul>
    ...
    <li type="none" style="color: #153643; padding: 5px 10px 7px 7px">
        <a href="${resumeRequest.url}">${resumeRequest.id}</a> - ${resumeRequest.company} - ${resumeRequest.name} -
        ${resumeRequest.status} - ${resumeRequest.priority}
        <ul>
...
<ul>
    ...
    <li type="none" style="color: #153643; padding: 5px 10px 7px 7px">
        <a href="${estimationRequest.url}">${estimationRequest.id}</a> - ${estimationRequest.company} -
        ${estimationRequest.name} - ${estimationRequest.status}
    </li>
</#list>
</ul>
<#else>
